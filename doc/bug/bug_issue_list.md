...
| 2026-02-06 | - | Bug #10: **"LET IT FLOW" TEST** - Added debug mode to continue detection for all 101 frames. CuteBaby sim 0.27-0.35 with 64% match rate. Frame-to-frame variance ±0.04 even with constant lighting. |
| 2026-02-06 | - | Bug #10: **NICEDADDY DISTANCE TEST** - Live person shows same issue. Far: sim 0.07-0.15 (no match). Medium: sim 0.15-0.28 (no match). Close: sim 0.28-0.32 (barely matches). Hailo requires close-range faces. |
| 2026-02-07 | - | Bug #10: **PI REBOOT FIXES HAILO** - Full Pi reboot restored Hailo performance (sim 0.31-0.36, 98% match rate). Greengrass-only restart does NOT fix. Evidence of Hailo VDevice state degradation over runtime. |
...
| 2026-02-08 | - | Bug #10: **MOBILEFACENET THRESHOLD TEST** - Free walking test with arcface_mobilefacenet. Threshold is pre_norm≥5.5 with **gradual transition** (not cliff): <5=0%, 5-5.5=3%, 5.5-6=39%, 6-7=73%, 7+=100%. mobilefacenet is more distance-tolerant than r50 despite smaller model size. |
