...
|---|---|---|---|
| 1 | Occupancy trigger + match | PASS | Rulin sim=0.4615, 7 frames, 1009ms, DC010 unlocked |
...
| 5 | Occupancy → no match → stop detection | PASS | Covers 6a: detection stopped early, 100 frames, 9857ms, identified: False |
...
| # | Scenario | Status | Notes |
|---|---|---|---|
| 2h | Entry button + match (Hailo) | | Re-register face with Hailo ArcFace, then button click → match → unlock |
...
**Verify**:
- [x] PY log: `trigger_face_detection` with `lock_asset_id` = DC010's assetId (`0x98a316fffe8e7d80`)
- [x] PY log: `MATCH` — Rulin sim=0.4615, 7 frames
- [x] Snapshot uploaded to S3
- [x] `member_detected` payload has `clickedLocks: ["0x98a316fffe8e7d80"]`
- [x] DC010 lock toggles (`zigbee2mqtt/DC010/set` → TOGGLE)
- [x] No `onvifTriggered` field in payload (removed)

**Result**: PASS (2026-02-25 15:14:22 JST, InsightFace)

**Log trace**:
```
TS: zigbee2mqtt/DC010 → occupancy: true
TS: handleLockTouchEvent → trigger_detection { cam_ip: "192.168.11.62", lock_asset_id: "0x98a316fffe8e7d80" }

PY: trigger_face_detection - started for camera: 192.168.11.62
PY: MATCH Rulin sim=0.4615, 7 frames, trigger_to_identified: 1009ms
PY: SESSION END - frames: 7, identified: True, duration: 909ms
PY: snapshot uploaded to S3
PY: member_detected published with clickedLocks: ["0x98a316fffe8e7d80"]

TS: unlockByMemberDetected → clickedLocks: ["0x98a316fffe8e7d80"]
TS: unlocking specific lock: 0x98a316fffe8e7d80
TS: zigbee2mqtt/DC010/set → TOGGLE

TS: occupancy: false → stop_detection (arrived after match, PY: no context — already cleaned up)
```
...
## Test 2h: Entry Button + Match (Hailo)

**Applies to**: Hailo backend only
**Trigger**: Press MAG001_ENTRY button (after re-registering face with Hailo ArcFace)
**Expected**: Detection starts → Hailo NPU detects face → `find_match()` crosses `FACE_THRESHOLD_HAILO` → `member_detected` → unlock MAG001

**Pre-requisite**: Re-register face via face registration API while running Hailo backend, so stored embeddings are generated by Hailo's ArcFace (not InsightFace). InsightFace embeddings are incompatible with Hailo ArcFace matching.

**Steps**:
1. Switch to `INFERENCE_BACKEND=hailo`, restart PY handler
2. Re-register face → new embedding stored in DynamoDB (Hailo ArcFace)
3. Press MAG001_ENTRY → TS publishes `trigger_detection { lock_asset_id: MAG001 }`
4. PY handler: `trigger_face_detection()` → starts detection with Hailo backend
5. `face_recognition_hailo.py` `process_frame()`: SCRFD detects face, ArcFace extracts embedding
6. `pre_norm` gate passes (face at normal distance)
7. `find_match()` → similarity crosses `FACE_THRESHOLD_HAILO` → MATCH
8. `DefaultMatchHandler.on_match()` → snapshot + `member_detected`
9. TS handler: `unlockByMemberDetected()` → unlock MAG001

**Verify**:
- [ ] PY log: Hailo backend active (`Thread-HailoDetector`)
- [ ] PY log: `pre_norm` above threshold (not skipped)
- [ ] PY log: `MATCH` with member name and sim above `FACE_THRESHOLD_HAILO`
- [ ] `member_detected` payload has `clickedLocks: [MAG001's assetId]`
- [ ] MAG001 lock toggles
...
**Verify**:
- [x] PY log: `trigger_face_detection - started` on occupancy=true
- [x] PY log: `handle_occupancy_false` on occupancy=false
- [x] PY log: `stopping detection early for: 192.168.11.62` — simplified logic, no `has_legacy` check
- [x] PY log: context after removal: `specific_locks: set(), active_occupancy: set()`
- [x] SESSION END: `frames: 100, identified: False, duration: 9857ms`

**Result**: PASS (2026-02-25 15:07:56 JST, InsightFace)

**Log trace**:
```
TS: zigbee2mqtt/DC010 → occupancy: true
TS: handleLockTouchEvent → trigger_detection { lock_asset_id: "0x98a316fffe8e7d80" }

PY: trigger_face_detection - started for camera: 192.168.11.62

TS: zigbee2mqtt/DC010 → occupancy: false (~10s later)
TS: handleLockStopEvent → stop_detection { lock_asset_id: "0x98a316fffe8e7d80" }

PY: handle_occupancy_false - context after removal: {specific_locks: set(), active_occupancy: set()}
PY: handle_occupancy_false - stopping detection early for: 192.168.11.62
PY: SESSION END - frames: 100, identified: False, duration: 9857ms
```
